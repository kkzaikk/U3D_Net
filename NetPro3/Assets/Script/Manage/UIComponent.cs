...
public class UIComponent //: MonoBehaviour
{
    ...
    //    foreach (Transform child in arrobj)
    //    {
    //        objMap[child.gameObject.name] = child.gameObject;
    //    }
    //}
    ...
    public virtual void NetResponse(int dest, int cmd) { }
    ...
    public bool UpdatePerFrame(float dt, ref int dest, int cmd)
    {
        ...
        {
            LogicUpdate(dt);
            if (acceptNet && dest != MessageStruct.NETACCEPTED)
            {
                NetResponse(dest, cmd);
