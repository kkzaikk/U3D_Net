...
class UILayerManage
{
    ...
    //keepInfo=true保持位置与角度信息111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111
    ...
    //实例化一个ui预设节点
    public static UIComponent InstantiateUIPre(UIComponent uiLayer, string _uiname)
    {
        UnityEngine.Object obj = Load(_uiname);
        if (obj != null)
        {
            GameObject _instObj = Instantiate(obj);
            if (_instObj == null)
            {
            ...
            uiLayer.UiName = _uiname;
            uiLayer.gameObject = _instObj;
            uiLayer.transform = _instObj.transform;
            Transform[] arrobj = _instObj.GetComponentsInChildren<Transform>(true);
            foreach (Transform ttt in arrobj)
            {
                uiLayer.ParseNode(ttt.gameObject);
            }
    ...
    public UIComponent CreateAndAdd(UIComponent uiLayer, string uiname, bool replace = true, LastLayerStatus s_LLStatus = LastLayerStatus.Active_N_Net_N)
    {
        if (currLayerComp != null)
        {
            if (currLayerComp.UiName == uiname)
            {
                Debug.Log("T CreateAndAdd 同样的界面不要连续加两次！" + uiname);
                return currLayerComp;
            }
        }

        //实例化
        UIComponent s_UILayer = UILayerManage.InstantiateUIPre(uiLayer, uiname);
    ...
    //增加一层界面
    private void AddNewLayer(UIComponent layer, LastLayerStatus s_LLStatus)
    {
        if (uiRoot == null)
        {
    ...
    //用新层替换当前层
    private void ReplaceLayer(UIComponent layer)
    {
        if (uiRoot == null)
        {
            uiRoot = GameObject.Find("UI Root").transform;
        ...
        if (currLayerComp == null)
        {
            AddNewLayer(layer, LastLayerStatus.Active_N_Net_N);
            return;
        }
        ...
        else
        {
            ...
            layer.PrevLayer = currLayerComp.PrevLayer;
    ...
    //删除一层
    public void RemoveLayer(UIComponent layer, bool jsremove = false)
    {
        if (layer != null) currLayerComp = layer;
