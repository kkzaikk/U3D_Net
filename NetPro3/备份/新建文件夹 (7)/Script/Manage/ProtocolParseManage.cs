...
//协议解析、打包管理类
public class ProtocolParseManage
{
    private static ProtocolParseManage s_protocolManage = null;
    public static ProtocolParseManage getInstance()
    {
        if (s_protocolManage == null)
        {
            s_protocolManage = new ProtocolParseManage();
            s_protocolManage.Init();
        }
        return s_protocolManage;
    }


    private Dictionary<string, ProtocolParseBasic> dictPBParseBasic = new Dictionary<string, ProtocolParseBasic>();

    public Dictionary<string, ProtocolParseBasic> getPBdict()
    {
    ...
    public void Init()
    {
        CurrProtocolManage.getInstance().onDispense += DispenseParseData;
    ...
    //分发协议数据到Model
    public void DispenseParseData(MessageStruct message)
    {
        string msgindex = "Msg_";
        msgindex += message.dest + "_";
        msgindex += message.cmd + "_";

        if (dictPBParseBasic.ContainsKey(msgindex))
        {
            dictPBParseBasic[msgindex].ParseData(msgindex, message.body);
        }
        else
        {
            // no add!!!!!!!!!!!!!!!
